...
pub struct NavigationAgentPlugin;

impl Plugin for NavigationAgentPlugin {
    fn build(&self, app: &mut App) {
        app .add_system_to_stage(GodotStages::Add, add_nodes::<NavigationAgent>)
            .add_system_to_stage(GodotStages::Sync, sync_bevy_owned)
            .add_system_to_stage(GodotStages::Sync, sync_godot_owned);
    }
}

/// Returns true if the node can be casted to a navigation_agent.
pub fn is_navigation_agent(node: &gdnative::prelude::Node) -> bool {
    node.cast::<NavigationAgent>().is_some()
}

/// A bundle for NavigationAgents.
/// Contains all parent components as well.
#[derive(Bundle)]
pub struct GDNavigationAgentBundle {
    pub navigation_agent: GDNavigationAgent,
    pub node: GDNode,
    pub true_type: TrueNodeType,
}

impl Default for GDNavigationAgentBundle {
    fn default() -> Self {
        Self {
            navigation_agent: Default::default(),
            node: Default::default(),
            true_type: TrueNodeType {
                node: None,
                class_name: "NavigationAgent".to_string()
            }
        }
    }
}

/// Represents a NavigationAgent.
#[derive(Component)]
pub struct GDNavigationAgent {
    pub agent_height_offset: f64,
...
impl Default for GDNavigationAgent {
    fn default() -> Self {
        Self {
            agent_height_offset: Default::default(),
avoidance_enabled: Default::default(),
ignore_y: Default::default(),
...
impl NodeClass for GDNavigationAgent {
    type Parent = GDNode;
    type GodotClass = NavigationAgent;

    fn add_components<T: WorldCommands>(world_commands: &mut T, entity: Entity, node: &gdnative::prelude::Node) {
        let component_ref = node.cast::<NavigationAgent>().unwrap();
        world_commands.insert(entity, GDNavigationAgent {
            agent_height_offset: component_ref.agent_height_offset(),
avoidance_enabled: component_ref.avoidance_enabled(),
ignore_y: component_ref.ignore_y(),
...
impl GDNavigationAgent {
    
}

fn sync_bevy_owned(query: Query<(&GDNavigationAgent, &TrueNodeType), With<BevyOwned>>) {
    for (component, true_type) in query.iter() {
        let component_ref = unsafe { true_type.node.unwrap().assume_safe() }.cast::<NavigationAgent>().unwrap();
        component_ref.set_agent_height_offset(component.agent_height_offset);
component_ref.set_avoidance_enabled(component.avoidance_enabled);
component_ref.set_ignore_y(component.ignore_y);
...
fn sync_godot_owned(mut query: Query<(&mut GDNavigationAgent, &TrueNodeType), With<GodotOwned>>) {
    for (mut component, true_type) in query.iter_mut() {
        let component_ref = unsafe { true_type.node.unwrap().assume_safe() }.cast::<NavigationAgent>().unwrap();
        component.agent_height_offset = component_ref.agent_height_offset();
component.avoidance_enabled = component_ref.avoidance_enabled();
component.ignore_y = component_ref.ignore_y();
